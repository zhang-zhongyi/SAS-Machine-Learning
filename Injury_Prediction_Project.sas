...
proc freq data=classifier1 order=freq;
	tables diag bdpt;
run;
...
data classifier2; set classifier1;
	format mydef mytest casef.;
	if index(narrative,'FELL') > 0 or index(narrative, 'ABRA') > 0 or index(narrative, 'CONTUS')>0
	then mytest = 1;
		else mytest=2;
...
data classifier3; set classifier1;
	format mydef mytest casef.;
...
data classifier4; set classifier1;
	format mydef mytest casef.;
...
data classifier5; set classifier1;
	format mydef mytest casef.;
	if (index(narrative,'FELL') > 0 or index(narrative,'FRACT') > 0 or index(narrative,'BONE') > 0 
...
data classifier6; set classifier1;
	format mydef mytest casef.;
...
data classifier7; set classifier1;
	format mydef mytest casef.;
